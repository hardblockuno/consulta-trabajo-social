...
export default function BookingSection() {
  return (
    <section id="reserva" className="bg-stone-50 py-14 sm:py-16">
      <div className="section-shell">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {bookingServices.map((service) => (
              <span
                key={service}
                className="rounded-md border border-stone-200 bg-white px-3 py-2 text-sm font-bold text-slate-700"
              >
                {service}
              </span>
            ))}
        ...
        <div id="profesionales" className="mt-8 grid gap-4 lg:grid-cols-2">
          {professionals.map((professional) => (
            <article key={professional.id} className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
              <div className="flex items-start gap-4">
                {professional.photo ? (
                  ...
                  <div className="grid h-24 w-20 flex-none place-items-center rounded-md bg-brand-900 text-base font-black text-white">
              ...
              <div className="mt-5 grid gap-3 text-sm font-bold text-slate-700 sm:grid-cols-3">
                <span className="inline-flex items-center gap-2 rounded-md bg-stone-50 px-3 py-2">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {professional.specialties.map((specialty) => (
                  <span
                    key={specialty}
                    className="rounded-md border border-stone-200 px-3 py-2 text-xs font-bold text-slate-700"
                  >
                    {specialty}
                  </span>
                ))}
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {professional.nextSlots.map((slot) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="focus-ring rounded-md border border-stone-200 bg-stone-50 px-2 py-3 text-center text-sm font-black text-slate-800 transition hover:border-brand-500 hover:bg-brand-50"
                    >
                      {slot}
                    </a>
                  ))}
